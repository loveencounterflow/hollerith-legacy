...
(function() {
  ...
  this._feed_test_data = function(db, probes_idx, settings, handler) {
    ...
    step((function(_this) {
      return function*(resume) {
        ...
        switch (probes_idx) {
          case -1:
            input.pipe(HOLLERITH.$write(db, settings)).pipe(D.$on_finish(function() {
  ...
  this["invalid key not accepted (1)"] = function(T, done) {
    ...
    return domain.run(function() {
      var input;
      input = D.new_stream();
      input.pipe(HOLLERITH.$write(db)).pipe(D.$on_finish(function() {
        return later(done);
      }));
      input.write('xxx');
      return input.end();
  ...
  this["building PODs from SPO phrases"] = function(T, done) {
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        return input.pipe($shorten_spo()).pipe($consolidate()).pipe(D.$show()).pipe(HOLLERITH.$write(db)).pipe(D.$on_finish(done));
      };
    })(this));
  };
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        try_writing = function() {
          var input;
          input = D.new_stream();
          input.pipe(D.$show()).pipe(HOLLERITH.$write(db)).pipe(D.$on_finish(function() {
  ...
  this["write private types (1)"] = function(T, done) {
    ...
    write_data = (function(_this) {
      return function(handler) {
        ...
        input = D.new_stream();
        input.pipe(HOLLERITH.$write(db)).pipe(D.$on_finish(handler));
        ref = _this._feed_test_data.probes[probes_idx];
        for (i = 0, len = ref.length; i < len; i++) {
          probe = ref[i];
          input.write(probe);
  ...
  this["write private types (2)"] = function(T, done) {
    ...
    write_data = (function(_this) {
      return function(handler) {
        var i, input, len, probe, ref;
        input = D.new_stream();
        input.pipe(HOLLERITH.$write(xdb)).pipe(D.$on_finish(handler));
        ref = _this._feed_test_data.probes[probes_idx];
        for (i = 0, len = ref.length; i < len; i++) {
          probe = ref[i];
          input.write(probe);
  ...
  this["(v4) new_longphrasestream accepts legal arguments"] = function(T, done) {
    var error, error1, i, len, probe, probe_idx, probes;
    probes = [
      {
        lo: ['xxx'],
        hi: ['xxx']
