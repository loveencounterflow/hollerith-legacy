...
(function() {
  ...
  this["(v4) secondary indexing, API"] = function(T, done) {
    var show, write_data;
    write_data = function(handler) {
      var IDX, i, input, len, probe, probes;
      IDX = Symbol["for"]('index');
      input = D.new_stream();
      input.pipe(HOLLERITH.$index('reading', 'usagecode')).pipe(HOLLERITH.$write(db, {
        unique: false,
        batch: 4
      })).pipe(D.$on_finish(handler));
      probes = CND.shuffle([['國', 'reading', 'guo'], ['千', 'reading', 'qian'], ['千', 'reading', 'bar'], ['千', 'reading', 'foo'], ['千', 'variant', '仟'], ['千', 'usagecode', 'CJKTHM'], ['千', 'gloss', 0, 'thousand']]);
      for (i = 0, len = probes.length; i < len; i++) {
        probe = probes[i];
        D.send(input, probe);
      }
  ...
  if (module.parent == null) {
    include = ["(v4) secondary indexing, manual", "(v4) secondary indexing, API"];
    this._prune();
    this._main();
  }
