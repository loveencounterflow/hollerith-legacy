...
(function() {
  var $, $async, CND, CODEC, D, HOLLERITH, SegfaultHandler, after, alert, badge, clear_leveldb, db, debug, echo, get_new_db_name, help, immediately, info, join, leveldown, levelup, log, njs_path, read_all_keys, rpr, show_db_entries, show_keys_and_key_bfrs, step, suspend, test, urge, warn, whisper, ƒ;
  ...
  this["catching errors (3)"] = function(T, done) {
    var d;
    return d = D.run(function() {
      var input, input1;
      input = D.create_throughstream();
      input1 = input.pipe(HOLLERITH.$write(db)).pipe(D.$on_end(function() {
        debug('©Yl1RN', d);
        return setImmediate(done);
      }));
      input.on('end', function() {
        return debug('©ueOU1-1', 'end');
      });
      input1.on('end', function() {
        return debug('©ueOU1-2', 'end');
      });
      input.write(['foo', 'bar', 'baz', 'gnu']);
      return input.end();
    }, function(error) {
