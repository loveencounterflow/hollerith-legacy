...
(function() {
  ...
  this["indexing"] = function(T, done) {
    ...
    $index = (function(_this) {
      return function(from_predicate, to_predicate, settings) {
        ...
        link = function(from_phrase, to_phrase) {
          var R, from_obj, from_prd, from_sbj, from_sub_obj, i, idx, j, len, len1, to_obj, to_prd, to_sbj, to_sbj_is_list, to_sub_obj;
          from_sbj = from_phrase[0], from_prd = from_phrase[1], from_obj = from_phrase[2];
          to_sbj = to_phrase[0], to_prd = to_phrase[1], to_obj = to_phrase[2];
          to_sbj_is_list = CND.isa_list(to_sbj);
          if (!(from_is_plural || to_is_plural)) {
            if (to_sbj_is_list) {
              R = [[slice.call(to_sbj).concat([to_prd], [to_obj]), from_prd, from_obj]];
            } else {
            ...
            R = [];
            if (to_is_plural) {
              for (i = 0, len = from_obj.length; i < len; i++) {
                from_sub_obj = from_obj[i];
                for (j = 0, len1 = to_obj.length; j < len1; j++) {
                  to_sub_obj = to_obj[j];
                  idx += +1;
                  if (to_sbj_is_list) {
                    R.push([slice.call(to_sbj).concat([to_prd], [idx], [to_sub_obj]), from_prd, from_sub_obj]);
                  } else {
    ...
    write_data = function(handler) {
      var input;
      input = D.create_throughstream();
      input.pipe($index('reading', 'similarity', {
        from: 'plural',
        to: 'plural'
        ...
        from: 'singular',
        to: 'plural'
      })).pipe($index('strokeorder', 'similarity', {
        from: 'singular',
        to: 'plural'
