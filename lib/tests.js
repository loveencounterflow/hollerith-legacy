...
(function() {
  var $, $async, ASYNC, CND, CODEC, D, HOLLERITH, after, alert, badge, clear_leveldb, db, debug, echo, get_new_db_name, help, include, info, isa_stream, join, later, leveldown, levelup, log, njs_path, read_all_keys, rpr, show_db_entries, show_keys_and_key_bfrs, step, suspend, test, urge, warn, whisper, ƒ,
  ...
  this._feed_test_data = function(db, probes_idx, settings, handler) {
    ...
    step((function(_this) {
      return function*(resume) {
        ...
        switch (probes_idx) {
          case -1:
            input.pipe(HOLLERITH.$write(db, settings)).pipe($('finish', function() {
  ...
  this["sort numbers with H2 codec (1)"] = function(T, done) {
    return step((function(_this) {
      return function*(resume) {
        ...
        leveldb = levelup('/tmp/hollerith2-test', settings);
        yield clear_leveldb(leveldb, resume);
        probes_and_descriptions = [[-2e308, "-Infinity"], [-Number.MAX_VALUE, "-Number.MAX_VALUE"], [Number.MIN_SAFE_INTEGER, "Number.MIN_SAFE_INTEGER"], [-123456789, "-123456789"], [-3, "-3"], [-2, "-2"], [-1.5, "-1.5"], [-1, "-1"], [-Number.EPSILON, "-Number.EPSILON"], [-Number.MIN_VALUE, "-Number.MIN_VALUE"], [0, "0"], [+Number.MIN_VALUE, "+Number.MIN_VALUE"], [+Number.EPSILON, "+Number.EPSILON"], [+1, "+1"], [+1.5, "+1.5"], [+2, "+2"], [+3, "+3"], [+123456789, "+123456789"], [Number.MAX_SAFE_INTEGER, "Number.MAX_SAFE_INTEGER"], [Number.MAX_VALUE, "Number.MAX_VALUE"], [+2e308, "+Infinity"]];
        matchers = (function() {
          var i, len, results;
          results = [];
  ...
  this["sort mixed values with H2 codec"] = function(T, done) {
    return step((function(_this) {
      return function*(resume) {
        ...
        leveldb = levelup('/tmp/hollerith2-test', settings);
        yield clear_leveldb(leveldb, resume);
        probes = [null, false, true, CODEC['sentinels']['firstdate'], new Date(0), new Date(8e11), new Date(), CODEC['sentinels']['lastdate'], 1234, 2e308, '', '一', '三', '二', '𠀀', '𠀀\x00', String.fromCodePoint(0x10ffff)];
        matchers = (function() {
          var i, len, results;
          results = [];
          for (i = 0, len = probes.length; i < len; i++) {
  ...
  this["sort lists of mixed values with H2 codec"] = function(T, done) {
    return step((function(_this) {
      return function*(resume) {
        ...
        leveldb = levelup('/tmp/hollerith2-test', settings);
        yield clear_leveldb(leveldb, resume);
        probes = [["", ''], ["1234", 1234], ["Infinity", 2e308], ["String.fromCodePoint 0x10ffff", String.fromCodePoint(0x10ffff)], ["false", false], ["new Date 0", new Date(0)], ["new Date 8e11", new Date(8e11)], ["new Date()", new Date()], ["null", null], ["true", true], ["一", '一'], ["三", '三'], ["二", '二'], ["𠀀", '𠀀'], ["𠀀\x00", '𠀀\x00']];
        matchers = (function() {
          var i, len, results;
          results = [];
          for (i = 0, len = probes.length; i < len; i++) {
  ...
  this["sort routes with values (2)"] = function(T, done) {
    return step((function(_this) {
      return function*(resume) {
        ...
        leveldb = levelup('/tmp/hollerith2-test', settings);
        yield clear_leveldb(leveldb, resume);
        probes = [['a', null], ['a', false], ['a', true], ['a', new Date()], ['a', -2e308], ['a', +1234], ['a', +2e308], ['a', 'b'], ['a', 'b\x00'], ['a\x00', +1234], ['a\x00', 'b'], ['aa', +1234], ['aa', 'b'], ['aa', 'b\x00']];
        matchers = (function() {
          var i, len, results;
          results = [];
          for (i = 0, len = probes.length; i < len; i++) {
  ...
  this["invalid key not accepted (1)"] = function(T, done) {
    ...
    return domain.run(function() {
      var input;
      input = D.new_stream();
      input.pipe(HOLLERITH.$write(db)).pipe($('finish', (function(_this) {
        return function() {
          return later(done);
        };
      })(this)));
  ...
  this["building PODs from SPO phrases"] = function(T, done) {
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        return input.pipe($shorten_spo()).pipe($consolidate()).pipe(D.$show()).pipe(HOLLERITH.$write(db)).pipe($('finish', done));
      };
    })(this));
  };
  ...
  this["read phrases in lockstep"] = function(T, done) {
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        yield _this._feed_test_data(db, probes_idx, resume);
        input_1 = HOLLERITH.new_phrasestream(db, {
          prefix: ['pos', 'strokecount']
        });
        input_2 = HOLLERITH.new_phrasestream(db, {
          prefix: ['pos', 'componentcount']
        });
        input_3 = HOLLERITH.new_phrasestream(db, {
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        try_writing = function() {
          var input;
          input = D.new_stream();
          input.pipe(D.$show()).pipe(HOLLERITH.$write(db)).pipe($('finish', (function(_this) {
            return function() {
  ...
  this["write private types (1)"] = function(T, done) {
    ...
    write_data = (function(_this) {
      return function(handler) {
        ...
        input = D.new_stream();
        input.pipe(HOLLERITH.$write(db)).pipe($('finish', handler));
        ref = _this._feed_test_data.probes[probes_idx];
        for (i = 0, len = ref.length; i < len; i++) {
          probe = ref[i];
          input.write(probe);
  ...
  this["write private types (2)"] = function(T, done) {
    ...
    write_data = (function(_this) {
      return function(handler) {
        var i, input, len, probe, ref;
        input = D.new_stream();
        input.pipe(HOLLERITH.$write(xdb)).pipe($('finish', handler));
        ref = _this._feed_test_data.probes[probes_idx];
        for (i = 0, len = ref.length; i < len; i++) {
          probe = ref[i];
          input.write(probe);
  ...
  this["(v4) use non-string subjects in phrases (1)"] = function(T, done) {
    var read, show, write_data;
    write_data = function(handler) {
      var IDX, input;
    ...
    read = function(settings, handler) {
      var Z, input;
      Z = [];
      input = HOLLERITH.new_phrasestream(db, settings);
      return input.pipe($((function(_this) {
        return function(phrase) {
          return Z.push(phrase);
        };
    ...
    show = function(handler) {
      ...
      tasks.push((function(_this) {
        return function(handler) {
          ...
          return input.pipe($(function(phrase) {
            return urge('1 (ubx:n flt:n)', JSON.stringify(phrase));
          })).pipe($('finish', function() {
            urge();
            return handler();
          }));
      ...
      tasks.push((function(_this) {
        return function(handler) {
          ...
          return input.pipe($(function(phrase) {
            return urge('2 (ubx:n flt:y)', JSON.stringify(phrase));
          })).pipe($('finish', function() {
            urge();
            return handler();
          }));
      ...
      tasks.push((function(_this) {
        return function(handler) {
          ...
          return input.pipe($(function(phrase) {
            return urge('3 (ubx:y flt:n)', JSON.stringify(phrase));
          })).pipe($('finish', function() {
            urge();
            return handler();
          }));
      ...
      tasks.push((function(_this) {
        return function(handler) {
          ...
          return input.pipe($(function(phrase) {
            return urge('4 (ubx:y flt:y)', JSON.stringify(phrase));
          })).pipe($('finish', function() {
            urge();
            return handler();
          }));
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        T.eq(probes, [["pos", "kwic/sortcode", null, "34d###", ["千"]], ["pos", "kwic/sortcode", null, "34d###", ["千", "reading", "foo"]], ["pos", "kwic/sortcode", null, "34d###", ["千", "shape/similarity", "于"]], ["spo", ["千"], "kwic/sortcode", null, "34d###"]]);
        probes = (yield read({
          unbox: false,
          flatten: true
        }, resume));
  ...
  this["(v4) secondary indexing, manual"] = function(T, done) {
    var show, write_data;
    write_data = function(handler) {
      var IDX, input;
      IDX = Symbol["for"]('index');
    ...
    show = function(handler) {
      ...
      return input.pipe($((function(_this) {
        return function(phrase) {
          return (phrase[0] === 'pos' ? urge : help)(JSON.stringify(phrase));
        };
      })(this))).pipe($('finish', (function(_this) {
        return function() {
          info();
  ...
  this["(v4) secondary async reads"] = function(T, done) {
    var matchers, read_data, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
      input.pipe(HOLLERITH.$write(db, {
    ...
    matchers = [['bar', '千'], ['bar', '千', ['gloss', 'thousand']], ['bar', '千', ['gloss', 'kilo']], ['bar', '千', ['gloss', 'millenary']], ['bar', '千', ['reading', 'foo']], ['bar', '千', ['reading', 'qian']], ['bar', '千', ['similarity', '于']], ['bar', '千', ['similarity', '干']], ['bar', '千', ['usagecode', 'CJKTHM']], ['bar', '千', ['variant', '仟']], ['bar', '千', ['variant', '韆']], ['foo', '千'], ['foo', '千', ['gloss', 'thousand']], ['foo', '千', ['gloss', 'kilo']], ['foo', '千', ['gloss', 'millenary']], ['foo', '千', ['reading', 'bar']], ['foo', '千', ['reading', 'qian']], ['foo', '千', ['similarity', '于']], ['foo', '千', ['similarity', '干']], ['foo', '千', ['usagecode', 'CJKTHM']], ['foo', '千', ['variant', '仟']], ['foo', '千', ['variant', '韆']], ['guo', '國'], ['guo', '國', ['variant', '囯']], ['guo', '國', ['variant', '国']], ['guo', '國', ['variant', '圀']], ['qian', '千'], ['qian', '千', ['gloss', 'thousand']], ['qian', '千', ['gloss', 'kilo']], ['qian', '千', ['gloss', 'millenary']], ['qian', '千', ['reading', 'bar']], ['qian', '千', ['reading', 'foo']], ['qian', '千', ['similarity', '于']], ['qian', '千', ['similarity', '干']], ['qian', '千', ['usagecode', 'CJKTHM']], ['qian', '千', ['variant', '仟']], ['qian', '千', ['variant', '韆']]];
    read_data = function(handler) {
      var input;
      input = HOLLERITH.new_phrasestream(db);
      return input.pipe($async((function(_this) {
        return function(phrase, send, end) {
          var _, obj, prd, sbj, selected, sub_input;
          selected = false;
          if (phrase != null) {
            if (phrase[0] === 'pos') {
              _ = phrase[0], prd = phrase[1], _ = phrase[2], obj = phrase[3], sbj = phrase[4];
              if (prd === 'reading') {
                selected = true;
                send([obj, sbj]);
                sub_input = HOLLERITH.new_phrasestream(db, {
                  prefix: ['spo', [sbj]],
                  star: '*'
                });
                sub_input.pipe($(function(sub_phrase) {
                  var sub_obj, sub_prd, sub_sbj;
                  _ = sub_phrase[0], sub_sbj = sub_phrase[1], sub_prd = sub_phrase[2], _ = sub_phrase[3], sub_obj = sub_phrase[4];
                  if (!CND.equals([sub_sbj, sub_prd, sub_obj], [sbj, prd, obj])) {
                    return send([obj, sbj, [sub_prd, sub_obj]]);
                  }
                })).pipe($('finish', function() {
                  return send.done();
                }));
        ...
        return function() {
          var idx;
          idx = -1;
          return $('null', function(phrase) {
            if (phrase != null) {
              idx += +1;
              return T.eq(phrase, matchers[idx]);
            } else {
              return T.eq(idx, matchers.length - 1);
            }
          });
        };
      })(this)()).pipe($('finish', handler));
  ...
  this["(v4) secondary indexing, API"] = function(T, done) {
    var matchers, probes, verify, write_data;
    probes = CND.shuffle([['國', 'reading', 'guo'], ['國', 'variant', '国'], ['國', 'variant', '圀'], ['國', 'variant', '囯'], ['國', 'usagecode', 'JKTHM'], ['千', 'reading', 'qian'], ['千', 'reading', 'bar'], ['千', 'reading', 'foo'], ['千', 'variant', '仟'], ['千', 'variant', '韆'], ['千', 'similarity', '于'], ['千', 'similarity', '干'], ['千', 'usagecode', 'CJKTHM'], ['千', 'gloss', 0, 'thousand'], ['千', 'gloss', 1, 'kilo'], ['千', 'gloss', 2, 'millenary']]);
    matchers = [["pos", "gloss", null, "kilo", ["千"]], ["pos", "gloss", null, "millenary", ["千"]], ["pos", "gloss", null, "thousand", ["千"]], ["pos", "gloss", 0, "thousand", ["千"]], ["pos", "gloss", 1, "kilo", ["千"]], ["pos", "gloss", 2, "millenary", ["千"]], ["pos", "reading", null, "bar", ["千"]], ["pos", "reading", null, "bar", ["千", "gloss", null, "kilo"]], ["pos", "reading", null, "bar", ["千", "gloss", null, "millenary"]], ["pos", "reading", null, "bar", ["千", "gloss", null, "thousand"]], ["pos", "reading", null, "bar", ["千", "gloss", 0, "thousand"]], ["pos", "reading", null, "bar", ["千", "gloss", 1, "kilo"]], ["pos", "reading", null, "bar", ["千", "gloss", 2, "millenary"]], ["pos", "reading", null, "bar", ["千", "reading", null, "foo"]], ["pos", "reading", null, "bar", ["千", "reading", null, "qian"]], ["pos", "reading", null, "bar", ["千", "similarity", null, "于"]], ["pos", "reading", null, "bar", ["千", "similarity", null, "干"]], ["pos", "reading", null, "bar", ["千", "usagecode", null, "CJKTHM"]], ["pos", "reading", null, "bar", ["千", "variant", null, "仟"]], ["pos", "reading", null, "bar", ["千", "variant", null, "韆"]], ["pos", "reading", null, "foo", ["千"]], ["pos", "reading", null, "foo", ["千", "gloss", null, "kilo"]], ["pos", "reading", null, "foo", ["千", "gloss", null, "millenary"]], ["pos", "reading", null, "foo", ["千", "gloss", null, "thousand"]], ["pos", "reading", null, "foo", ["千", "gloss", 0, "thousand"]], ["pos", "reading", null, "foo", ["千", "gloss", 1, "kilo"]], ["pos", "reading", null, "foo", ["千", "gloss", 2, "millenary"]], ["pos", "reading", null, "foo", ["千", "reading", null, "bar"]], ["pos", "reading", null, "foo", ["千", "reading", null, "qian"]], ["pos", "reading", null, "foo", ["千", "similarity", null, "于"]], ["pos", "reading", null, "foo", ["千", "similarity", null, "干"]], ["pos", "reading", null, "foo", ["千", "usagecode", null, "CJKTHM"]], ["pos", "reading", null, "foo", ["千", "variant", null, "仟"]], ["pos", "reading", null, "foo", ["千", "variant", null, "韆"]], ["pos", "reading", null, "guo", ["國"]], ["pos", "reading", null, "guo", ["國", "usagecode", null, "JKTHM"]], ["pos", "reading", null, "guo", ["國", "variant", null, "囯"]], ["pos", "reading", null, "guo", ["國", "variant", null, "国"]], ["pos", "reading", null, "guo", ["國", "variant", null, "圀"]], ["pos", "reading", null, "qian", ["千"]], ["pos", "reading", null, "qian", ["千", "gloss", null, "kilo"]], ["pos", "reading", null, "qian", ["千", "gloss", null, "millenary"]], ["pos", "reading", null, "qian", ["千", "gloss", null, "thousand"]], ["pos", "reading", null, "qian", ["千", "gloss", 0, "thousand"]], ["pos", "reading", null, "qian", ["千", "gloss", 1, "kilo"]], ["pos", "reading", null, "qian", ["千", "gloss", 2, "millenary"]], ["pos", "reading", null, "qian", ["千", "reading", null, "bar"]], ["pos", "reading", null, "qian", ["千", "reading", null, "foo"]], ["pos", "reading", null, "qian", ["千", "similarity", null, "于"]], ["pos", "reading", null, "qian", ["千", "similarity", null, "干"]], ["pos", "reading", null, "qian", ["千", "usagecode", null, "CJKTHM"]], ["pos", "reading", null, "qian", ["千", "variant", null, "仟"]], ["pos", "reading", null, "qian", ["千", "variant", null, "韆"]], ["pos", "similarity", null, "于", ["千"]], ["pos", "similarity", null, "干", ["千"]], ["pos", "usagecode", null, "CJKTHM", ["千"]], ["pos", "usagecode", null, "JKTHM", ["國"]], ["pos", "variant", null, "仟", ["千"]], ["pos", "variant", null, "囯", ["國"]], ["pos", "variant", null, "国", ["國"]], ["pos", "variant", null, "圀", ["國"]], ["pos", "variant", null, "韆", ["千"]], ["spo", ["千"], "gloss", 0, "thousand"], ["spo", ["千"], "gloss", 1, "kilo"], ["spo", ["千"], "gloss", 2, "millenary"], ["spo", ["千"], "reading", null, "bar"], ["spo", ["千"], "reading", null, "foo"], ["spo", ["千"], "reading", null, "qian"], ["spo", ["千"], "similarity", null, "于"], ["spo", ["千"], "similarity", null, "干"], ["spo", ["千"], "usagecode", null, "CJKTHM"], ["spo", ["千"], "variant", null, "仟"], ["spo", ["千"], "variant", null, "韆"], ["spo", ["國"], "reading", null, "guo"], ["spo", ["國"], "usagecode", null, "JKTHM"], ["spo", ["國"], "variant", null, "囯"], ["spo", ["國"], "variant", null, "国"], ["spo", ["國"], "variant", null, "圀"]];
    write_data = function(handler) {
      var IDX, i, input, len, probe;
      IDX = Symbol["for"]('index');
      input = D.new_stream();
      input.pipe(HOLLERITH.$index('reading', 'reading')).pipe(HOLLERITH.$index('reading', 'gloss')).pipe(HOLLERITH.$index('reading', 'similarity')).pipe(HOLLERITH.$index('reading', 'usagecode')).pipe(HOLLERITH.$index('reading', 'variant')).pipe(HOLLERITH.$write(db, {
        unique: false,
        batch: 4
      })).pipe($('finish', handler));
      for (i = 0, len = probes.length; i < len; i++) {
  ...
  this["(v4) n-ary indexing (2)"] = function(T, done) {
    ...
    show = function(handler) {
      ...
      return input.pipe($((function(_this) {
        return function(phrase) {
          return info('5543', JSON.stringify(phrase));
        };
      })(this))).pipe((function(_this) {
        return function() {
          var idx;
  ...
  this["(v4) values are `0x00` buffers"] = function(T, done) {
    var matcher, show, write_data;
  ...
  this["(v4) store SPO, POS both as keys only"] = function(T, done) {
    var show, value_matchers, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
      input.pipe(HOLLERITH.$write(db, {
    ...
    value_matchers = [[84, 112, 111, 115, 0, 84, 118, 97, 114, 105, 97, 110, 116, 0, 66, 84, 228, 187, 159, 0, 69, 84, 229, 141, 131, 0, 0], [84, 115, 112, 111, 0, 69, 84, 229, 141, 131, 0, 0, 84, 118, 97, 114, 105, 97, 110, 116, 0, 66, 84, 228, 187, 159, 0]];
    show = function(handler) {
      var input, query;
      query = {
        prefix: [],
      ...
      return input.pipe($((function(_this) {
        return function(record) {
          return info(JSON.stringify(record));
        };
      })(this))).pipe((function(_this) {
        return function() {
          var idx, zero_bfr;
          ...
          return $('null', function(record) {
            ...
            if (record != null) {
              ...
              return T.eq(value, zero_bfr);
            } else {
              return T.eq(idx, value_matchers.length - 1);
            }
          });
        };
      })(this)()).pipe($('finish', handler));
  ...
  this["(v4) store non-list subject as single-element list"] = function(T, done) {
    var show, value_matchers, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
    ...
    value_matchers = [[0x54, 0x70, 0x6f, 0x73, 0x00, 0x54, 0x76, 0x61, 0x72, 0x69, 0x61, 0x6e, 0x74, 0x00, 0x42, 0x54, 0xe4, 0xbb, 0x9f, 0x00, 0x45, 0x54, 0xe5, 0x8d, 0x83, 0x00, 0x00], [0x54, 0x73, 0x70, 0x6f, 0x00, 0x45, 0x54, 0xe5, 0x8d, 0x83, 0x00, 0x00, 0x54, 0x76, 0x61, 0x72, 0x69, 0x61, 0x6e, 0x74, 0x00, 0x42, 0x54, 0xe4, 0xbb, 0x9f, 0x00]];
    show = function(handler) {
      var input, query;
      query = {
        prefix: [],
  ...
  this["(v4) read POS phrases (sbj is a list)"] = function(T, done) {
    var matchers_boxed, matchers_unboxed, show, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
      input.pipe(HOLLERITH.$write(db, {
    ...
    matchers_boxed = [['pos', 'usagecode', null, 'CJKTHM', ['glyph', '千']], ['pos', 'variant', null, '仟', ['glyph', '千']], ['pos', 'variant', null, '韆', ['glyph', '千']]];
    matchers_unboxed = [['pos', 'usagecode', null, 'CJKTHM', ['glyph', '千']], ['pos', 'variant', null, '仟', ['glyph', '千']], ['pos', 'variant', null, '韆', ['glyph', '千']]];
    show = function(handler) {
      var boxed_has_ended, input_boxed, input_unboxed, query, unboxed_has_ended;
      boxed_has_ended = false;
      unboxed_has_ended = false;
      query = {
        prefix: ['pos'],
        star: '*'
      };
      input_boxed = HOLLERITH.new_phrasestream(db, query);
      input_boxed.pipe($((function(_this) {
        return function(phrase) {
          return info(rpr(phrase));
        };
      })(this))).pipe((function(_this) {
        return function() {
          var idx;
          ...
          return $('null', function(phrase) {
            if (phrase != null) {
              idx += +1;
              return T.eq(phrase, matchers_boxed[idx]);
            } else {
              boxed_has_ended = true;
              T.eq(idx, matchers_boxed.length - 1);
              if (unboxed_has_ended) {
                return handler();
              }
            }
          });
      ...
      return input_unboxed.pipe($((function(_this) {
        return function(phrase) {
          return info(rpr(phrase));
        };
      })(this))).pipe((function(_this) {
        return function() {
          ...
          return $('null', function(phrase) {
            if (phrase != null) {
              idx += +1;
              return T.eq(phrase, matchers_unboxed[idx]);
            } else {
              unboxed_has_ended = true;
  ...
  this["(v4) read POS phrases (sbj isn't a list)"] = function(T, done) {
    var matchers, show, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
    ...
    matchers = [['pos', 'usagecode', null, 'CJKTHM', '千'], ['pos', 'variant', null, '仟', '千'], ['pos', 'variant', null, '韆', '千']];
    show = function(handler) {
      var input, query;
      query = {
        prefix: ['pos'],
  ...
  this["(v4) read POS phrases (sbj is a singleton list)"] = function(T, done) {
    var matchers, show, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
      input.pipe(D.$show()).pipe(HOLLERITH.$write(db, {
        unique: false,
        batch: 4
    ...
    show = function(handler) {
      ...
      input = HOLLERITH.new_phrasestream(db, query);
      return pipeline = input.pipe($((function(_this) {
        return function(phrase) {
          if (phrase != null) {
            return info(rpr(phrase));
          }
  ...
  this["(v4) read normalized phrases"] = function(T, done) {
    T.fail("not implemented");
    return done();
  };

  this["dddddddddddddddddd"] = function(T, done) {
    var matchers, show, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
      ...
      input.write(['千', 'usagecode', 'CJKTHM']);
      input.write(['韆', 'usagecode', null, 'KTHm']);
      ...
      input.write([
        'room-012', 'temperature', new Date(2016, 3, 1, 13, 3, 56), {
          type: 'temperature',
          value: [17.4, '°C']
        }
      ]);
      input.write([
        'room-012', 'temperature', new Date(2016, 3, 1, 13, 4, 30), {
          type: 'temperature',
          value: [18.5, '°C']
        }
      ]);
      input.write([
        'room-012', 'temperature', new Date(2016, 3, 1, 13, 23, 2), {
          type: 'temperature',
          value: [16.1, '°C']
        }
      ]);
  ...
  this["eeeeeeeeeeeeeeeeee"] = function(T, done) {
    var matchers, show, write_data;
    write_data = function(handler) {
      var input;
      input = D.new_stream();
  ...
  this["(v4) new_longphrasestream rejects illegal arguments"] = function(T, done) {
    var i, len, matcher, matchers, probe, probe_idx, probes;
    probes = [
      (function() {
        return HOLLERITH.new_longphrasestream(db, {
          hi: ['xxx']
        });
      }), (function() {
        return HOLLERITH.new_longphrasestream(db, {
    ...
    matchers = ["must use either 'prefix' or 'hi' and 'lo', got 'hi'", "must use either 'prefix' or 'hi' and 'lo', got 'lo'", "illegal to use 'hi' or 'lo' together with 'prefix'", "legal query keys are 'prefix', 'star', 'lo', 'hi', 'unbox', 'flatten', got 'foobar'", "expected 'star' to be '*', got '?'", "must use either 'prefix' or 'hi' and 'lo', got 'lo', 'star'"];
    for (probe_idx = i = 0, len = probes.length; i < len; probe_idx = ++i) {
      probe = probes[probe_idx];
      matcher = matchers[probe_idx];
  ...
  this["(v4) new_longphrasestream accepts legal arguments"] = function(T, done) {
    var error, i, len, probe, probe_idx, probes;
    probes = [
      {
        lo: ['xxx'],
        hi: ['xxx']
  ...
  this["(v4) opening DB 2nd time returns reference to same object"] = function(T, done) {
    var db_1, db_2, error;
    try {
    ...
    if (db_1 === db_2) {
      T.succeed("DBs are identical");
    } else {
      T.fail("DBs are different");
    }
    return done();
  };
  ...
  if (module.parent == null) {
    include = ["(v4) values are `0x00` buffers", "(v4) store SPO, POS both as keys only", "(v4) store non-list subject as single-element list", "(v4) read POS phrases (sbj is a list)", "(v4) read POS phrases (sbj isn't a list)", "(v4) new_longphrasestream rejects illegal arguments", "(v4) new_longphrasestream accepts legal arguments", "(v4) read POS phrases (sbj is a singleton list)", "(v4) secondary async reads", "(v4) secondary indexing, manual", "(v4) secondary indexing, API", "(v4) opening DB 2nd time returns reference to same object"];
    this._prune();
    this._main();
  }
