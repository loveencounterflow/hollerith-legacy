...
(function() {
  ...
  this["building PODs from SPO phrases"] = function(T, done) {
    ...
    return step((function(_this) {
      return function*(resume) {
        var input_A, input_B, prefix;
        (yield _this._feed_test_data(db, probes_idx, resume));
        prefix = ['spo'];
        input_A = HOLLERITH.create_phrasestream(db, {
          prefix: prefix
        });
        return input_B = input_A.pipe($shorten_spo()).pipe($consolidate()).pipe(D.$show()).pipe(HOLLERITH.$write(db)).pipe(D.$on_end(function() {
          debug('©Lqzy1', '...');
