(function() {
  var $, $async, ASYNC, CHR, CND, D, HOLLERITH, after, alert, badge, debug, echo, eventually, every, help, immediately, info, join, log, new_db, njs_path, options, repeat_immediately, rpr, step, suspend, urge, warn, whisper, ƒ,
    slice = [].slice,
  ...
  this.create_phrasestream_for_selection = function(db, prd, filter) {};

  this.show_kwic_v2_sample = function(db) {
    return step((function(_this) {
      return function*(resume) {
        var db_route, factors, input, insert, prefix, query, rank_fallback, rank_limit, ranks, rotate_left, rotate_right, sample;
        db_route = join(__dirname, '../../jizura-datasources/data/leveldb-v2');
        if (db == null) {
          db = HOLLERITH.new_db(db_route, {
            create: false
        ...
        help("using DB at " + db['%self']['location']);
        factors = (yield _this.read_factors(db, resume));
        ...
        return input.pipe($(function(phrase, send) {
          ...
          var _, glyph, idx, sortcode;
          _ = phrase[0], _ = phrase[1], sortcode = phrase[2], glyph = phrase[3], idx = phrase[4];
          if ((CHR.as_csg(glyph, {
            input: 'xncr'
          })) !== 'u') {
            return;
          ...
          return indexOf.call(sample, glyph) >= 0;
        })).pipe($(function(arg, send) {
          var glyph, sortcode, sortrow, x;
          glyph = arg[0], sortcode = arg[1];
          ...
          sortrow = (function() {
            var i, len, results;
            results = [];
            for (i = 0, len = sortrow.length; i < len; i++) {
              x = sortrow[i];
              results.push(x.split(','));
            }
            return results;
          })();
          return send([glyph, sortrow]);
        })).pipe($(function(arg, send) {
          var entry, glyph, i, idx, len, new_sortrow, position, sortcode, sortrow;
          glyph = arg[0], sortrow = arg[1];

          /* shorten sortrow */
          new_sortrow = [];
          for (idx = i = 0, len = sortrow.length; i < len; idx = ++i) {
            entry = sortrow[idx];
            sortcode = entry[0], position = entry[1];
            if ((parseInt(position, 10)) < 6) {
              new_sortrow.push(entry);
            }
          }
          return send([glyph, new_sortrow]);
        })).pipe($(function(arg, send) {
          var _, entry, factor, glyph, i, idx, len, lineup, position, positions, ref, ref1, rotation, sortcode, sortrow;
          glyph = arg[0], sortrow = arg[1];
          lineup = [];
          positions = (function() {
            var i, len, ref, results;
            results = [];
            for (i = 0, len = sortrow.length; i < len; i++) {
              ref = sortrow[i], _ = ref[0], position = ref[1];
          ...
          for (idx = i = 0, len = sortrow.length; i < len; idx = ++i) {
            ...
            factor = (ref = (ref1 = factors[sortcode]) != null ? ref1['glyph'] : void 0) != null ? ref : '\u3000';
            entry.push(factor);
          }
          return send([glyph, sortrow, positions, rotation]);
        })).pipe($(function(arg, send) {
          var d, factor, glyph, i, len, lineup, position, positions, ref, rotation, sortcode, sortrow;
          glyph = arg[0], sortrow = arg[1], positions = arg[2], rotation = arg[3];
          d = [];
          lineup = [];
          for (i = 0, len = sortrow.length; i < len; i++) {
            ref = sortrow[i], sortcode = ref[0], position = ref[1], factor = ref[2];
            sortcode = sortcode.slice(0, 4).replace(/-/g, ' ');
          ...
          return send([glyph, sortrow, lineup]);
        })).pipe(D.$collect()).pipe($(function(collection_v1, send) {
          var _, collection_v2, entry, glyph, i, j, k, len, len1, len2, lineup, new_sortrow, position, ref, ref1, sortcode, sortrow;
          collection_v2 = CND.LODASH.cloneDeep(collection_v1);
          for (i = 0, len = collection_v2.length; i < len; i++) {
            entry = collection_v2[i];
            glyph = entry[0], sortrow = entry[1], lineup = entry[2];
            new_sortrow = [];
            for (j = 0, len1 = sortrow.length; j < len1; j++) {
              ref = sortrow[j], sortcode = ref[0], _ = ref[1], _ = ref[2];
              new_sortrow.push(sortcode);
            }
            for (k = 0, len2 = sortrow.length; k < len2; k++) {
              ref1 = sortrow[k], _ = ref1[0], position = ref1[1], _ = ref1[2];
              new_sortrow.push(position);
            }
            entry.push(new_sortrow.join(''));
          }
          collection_v2.sort(function(a, b) {
            if (a[3] < b[3]) {
              return -1;
            }
            if (a[3] > b[3]) {
          ...
          return send([collection_v1, collection_v2]);
        })).pipe($(function(arg, send) {
          var _, collection_v1, collection_v2, entry_v1, glyph_v1, glyph_v2, i, idx, len, lineup_v1, lineup_v2, ref, results;
          collection_v1 = arg[0], collection_v2 = arg[1];
          results = [];
          for (idx = i = 0, len = collection_v1.length; i < len; idx = ++i) {
            entry_v1 = collection_v1[idx];
            glyph_v1 = entry_v1[0], _ = entry_v1[1], lineup_v1 = entry_v1[2];
            ref = collection_v2[idx], glyph_v2 = ref[0], _ = ref[1], lineup_v2 = ref[2];
            help(lineup_v1, glyph_v1, '◉', lineup_v2, glyph_v2);
            results.push(echo(lineup_v1, glyph_v1, '◉', lineup_v2, glyph_v2));
          }
          return results;
        }));
      };
