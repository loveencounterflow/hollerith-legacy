(function() {
  var $, $async, ASYNC, CHR, CND, D, HOLLERITH, KWIC, after, alert, badge, debug, echo, eventually, every, help, immediately, info, join, log, new_db, njs_path, options, repeat_immediately, rpr, step, suspend, urge, warn, whisper, ƒ,
    slice = [].slice;
  ...
  this.show_kwic_v2_and_v3_sample = function(db) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input_v3 = (HOLLERITH.create_phrasestream(db, query_v3)).pipe($transform_v3()).pipe(D.$observe(function(arg) {
          var glyph, lineup;
          glyph = arg[0], lineup = arg[1];
          if (glyph === '畴') {
            return help(glyph, lineup);
          }
        }));
        input_v1.pipe(D.$lockstep(input_v2, {
          fallback: [null, null]
        })).pipe(D.$lockstep(input_v3)).pipe($unpack()).pipe((function() {
          var count, include_v1, spc;
          count = 0;
          spc = '\u3000';
          include_v1 = false;
          return D.$observe(function(arg) {
            var diff, diff_v1, diff_v2, glyph_v1, glyph_v2, glyph_v3, line, lineup_v1, lineup_v2, lineup_v3, v1, v2, v3;
            v1 = arg[0], v2 = arg[1], v3 = arg[2];
            glyph_v1 = v1[0], lineup_v1 = v1[1];
            glyph_v2 = v2[0], lineup_v2 = v2[1];
            glyph_v3 = v3[0], lineup_v3 = v3[1];
            diff = [];
            diff_v1 = glyph_v1 === glyph_v2 ? ' ' : '<';
            diff_v2 = glyph_v2 === glyph_v3 ? ' ' : '>';
            if (include_v1) {
