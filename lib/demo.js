(function() {
  ...
  this.show_kwic_v2_sample = function(db) {
    var $exclude_gaiji, $exclude_rare_glyphs, $extract_lineup, $include_sample, $reorder_phrase, $transform, db_route, input_v1, input_v2, prefix_v1, prefix_v2, rank_limit, ranks, sample;
    db_route = join(__dirname, '../../jizura-datasources/data/leveldb-v2');
    if (db == null) {
      db = HOLLERITH.new_db(db_route, {
        create: false
    ...
    $exclude_rare_glyphs = (function(_this) {
      return function() {
        return $async(function(arg, done) {
          var glyph, rank, sortcode, sub_prefix, sub_query;
          glyph = arg[0], sortcode = arg[1];

          /* filter out 'uncommon' glyphs (whose rank exceeds rank limit) */
          if (rank_limit < 0 || rank_limit === Infinity) {
            return done([glyph, sortcode]);
          }
    ...
    $transform = (function(_this) {
      return function() {
        return D.combine([$reorder_phrase(), $exclude_gaiji(), $exclude_rare_glyphs(), $extract_lineup()]);
      };
    })(this);
    prefix_v1 = {
    ...
    input_v1.pipe(D.$lockstep(input_v2)).pipe((function(_this) {
      return function() {
        var count;
        count = 0;
        return $(function(arg, send) {
          var glyph_v1, glyph_v2, line, lineup_v1, lineup_v2, ref, ref1, spc;
          (ref = arg[0], glyph_v1 = ref[0], lineup_v1 = ref[1]), (ref1 = arg[1], glyph_v2 = ref1[0], lineup_v2 = ref1[1]);
          spc = '\u3000';
  ...
  if (module.parent == null) {
    ...
    debug('©AoOAS', options);
    this.show_codepoints_with_missing_predicates();
  }
