    ...
    return step((function(_this) {
      return function(resume) {
        ...
        return input.pipe($async(function(phrase, done) {
          var _, glyph, lineup_length, sub_prefix;
          _ = phrase[0], glyph = phrase[1], _ = phrase[2], lineup_length = phrase[3];
          sub_prefix = ['spo', glyph, 'rank/cjt'];
          return HOLLERITH.read_phrases(db, sub_prefix, '*', null, function(error, sub_phrases) {
            ...
            if ((count = sub_phrases.length) !== 1) {
              return done.error(new Error("illegal phrase count " + count));
            }
          ...
          var glyph, lineup_length, rank, ref, sub_prefix;
