(function() {
  var $, CND, CODEC, D, DUMP, _codec_decode, _codec_encode, _new_level_db, badge, debug, echo, help, leveldown, log, repeat_immediately, rpr, step, suspend, urge, warn, whisper, ƒ,
  ...
  this.clear = function(db, handler) {
    ...
    tasks.push((function(_this) {
      return function(handler) {
        whisper("erasing DB...");
        return leveldown.destroy(route, function() {
          whisper("ok");
          return handler();
        });
      };
  ...
  this.as_url = function(db, key, value, settings) {
    ...
    debug('©iU0gA', this.as_phrase(db, key));
    debug('©iU0gA', this.normalize_phrase(db, this.as_phrase(db, key)));
