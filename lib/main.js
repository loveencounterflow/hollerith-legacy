...
(function() {
  var $, $async, $on_finish, CND, CODEC, D, DUMP, _codec_decode, _codec_encode, _codec_encode_plus_tm_hi, _new_level_db, badge, debug, echo, help, leveldown, log, rpr, step, suspend, urge, warn, whisper, ƒ,
    slice = [].slice,
  ...
  $ = D.$, $async = D.$async, $on_finish = D.$on_finish;
  ...
  this.$write = function(db, settings) {
    var $add_primary_index, $add_secondary_index, $encode, $write_batch, batch_size, batch_written, ensure_unique, is_integer, loner_predicates, pipeline, ref, ref1, ref2, ref3, solid_predicates, substrate;
    if (settings == null) {
      settings = {};
    }
    ...
    $add_secondary_index = (function(_this) {
      return function() {
        var cache;
        cache = [];
        return $async(function(phrase, send, end) {
          var ASYNC, _, fn, i, index_settings, len, tasks;
          if (phrase != null) {
            if (phrase[0] !== Symbol["for"]('make-secondary-index')) {
              return send.done(phrase);
            }
          ...
          if (end != null) {
            ...
            fn = function(index_settings) {
              return tasks.push(function(handler) {
                ...
                return _this._add_secondary_index(db, index_settings, function(error) {
                  if (error != null) {
                    return handler(error);
                  }
                  return handler();
            ...
            for (i = 0, len = cache.length; i < len; i++) {
              index_settings = cache[i];
              fn(index_settings);
            }

            /* TAINT arbitrary magic number for limit */
            ASYNC.parallelLimit(tasks, 10, function(error) {
              if (error != null) {
  ...
  this.$index_v4 = (function(_this) {
    return function() {
      ...
      return D.$on_start(function(send) {
        ...
        return send([Symbol["for"]('make-secondary-index'), index_settings]);
      });
    };
  })(this);

  this._add_secondary_index = (function(_this) {
    return function(db, index_settings, handler) {
      var from_prd, input, predicates, query, to_prd;
      predicates = index_settings.predicates;
      from_prd = predicates[0], to_prd = predicates[1];
      query = {
        prefix: [],
        star: '*',
        flatten: true
      };
      input = _this.new_phrasestream(db, query);
      return input.pipe($(function(phrase) {
        return log(CND.steel(predicates, JSON.stringify(phrase)));
      })).pipe($on_finish(function() {
        debug("finished: " + predicates);
        return handler();
      }));
    };
  })(this);
  ...
  this.new_phrasestream = function(db, query, settings) {
    var R;
    R = this._new_phrasestream(db, query, settings);
    return R;
  };
