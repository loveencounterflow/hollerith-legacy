(function() {
  var $, $async, CND, CODEC, D, DUMP, _codec_decode, _codec_encode, _new_level_db, badge, debug, echo, help, leveldown, log, repeat_immediately, rpr, step, suspend, urge, warn, whisper, ƒ,
    ...
    $index = (function(_this) {
      return function() {
        return $(function(spo, send) {
          ...
          if (!CND.isa_list(spo)) {
            return send.error(new Error("invalid SPO key, must be list: " + (rpr(spo))));
          }
          if (spo.length !== 3) {
            return send.error(new Error("invalid SPO key, must be of length 3: " + (rpr(spo))));
          }
      ...
      return function() {
        ...
        return $async(function(xphrase, done) {
          ...
          return _this.has(db, key_bfr, function(error, db_has_key) {
            ...
            if (db_has_key) {
              return done.error(new Error("phrase already in DB: " + (rpr(phrase))));
            }
            return done(xphrase);
          });
        });
