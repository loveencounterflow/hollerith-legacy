(function() {
  ...
  this.create_phrasestream = function(db, settings) {
    return this._create_phrasestream(db, settings);
  };

  this.read_phrases = function(db, settings, handler) {
    var arity;
    switch (arity = arguments.length) {
      case 2:
        handler = settings;
    ...
    return this._create_phrasestream(db, settings, handler);
  };

  this.read_one_phrase = function(db, settings, handler) {
    var arity, fallback;
    fallback = this._misfit;
    switch (arity = arguments.length) {
      case 2:
    ...
    if ((settings != null) && 'fallback' in settings) {
      fallback = settings['fallback'];
      delete settings['fallback'];
    }
    return this.read_phrases(db, settings, (function(_this) {
      return function(error, phrases) {
        ...
        if (phrases.length !== 1) {
          return handler(new Error("expected single phrase, got " + phrases.length));
        }
  ...
  this._create_phrasestream = function(db, settings, handler) {
    var R, input;
    input = this.create_facetstream(db, settings);
