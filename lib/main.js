...
(function() {
  ...
  this.create_phrasestream = function(db, query) {
    var R;
    R = this._create_phrasestream(db, query);
    return R;
  };
  ...
  this._create_phrasestream = function(db, query, handler) {
    ...
    R = input.pipe(this.$longphrase_as_phrase(db));
    if (handler != null) {
  ...
  this.longphrase_as_phrase = function(db, longphrase) {
    var error, error1, idx, length, longphrase_length, obj, phrasetype, prd, sbj, sbj_length, tail;
    try {
      phrasetype = longphrase[0], tail = 2 <= longphrase.length ? slice.call(longphrase, 1) : [];
      sbj = prd = obj = idx = null;
      switch (phrasetype) {
        case 'spo':
          if ((length = longphrase.length) !== 4) {
            throw new Error("illegal SPO phrase (length " + length + ")");
          }
          sbj = tail[0], prd = tail[1], obj = tail[2];
          break;
        case 'pos':
          switch (longphrase_length = longphrase.length) {
            case 4:
              prd = tail[0], obj = tail[1], sbj = tail[2];
              break;
            case 5:
  ...
  this.$longphrase_as_phrase = function(db) {
    return $((function(_this) {
      return function(key, send) {
        return send(_this.longphrase_as_phrase(db, key));
      };
    })(this));
  };
