(function() {
  var $, $async, CND, D, DEMO, HOLLERITH, KWIC, after, alert, badge, debug, echo, eventually, every, help, immediately, info, join, log, njs_path, options, repeat_immediately, rpr, step, suspend, urge, warn, whisper, ƒ,
    ...
    return $((function(_this) {
      return function(arg, send) {
        ...
        weights = (function() {
          var i, len, results;
          results = [];
          for (i = 0, len = sortrow_v1.length; i < len; i++) {
            x = sortrow_v1[i];
            results.push(x.split(','));
          }
          return results;
        ...
        weights = (function() {
          ...
          for (i = 0, len = weights.length; i < len; i++) {
            ref2 = weights[i], sortcode = ref2[0], _ = ref2[1];
